...
public void OnPluginStart() {
	...
	g_Cvar_JoinLimit = CreateConVar("sm_tl_joinlimit", "24", "Default total teamlimit");

	CreateConVar("sm_tl_version", PLUGIN_VERSION, "Version of Plugin. Do not change!");
	g_Cvar_TeamLimitDebug = CreateConVar("sm_tl_debug", "0.0", "Debug mode");
...
public Action Command_LimitInfo(int client, int args) {
	...
	int clientScore = GetEntProp(GetPlayerResourceEntity(), Prop_Send, "m_iTotalScore", _, client);
	...
	ReplyToCommand(client, "[TL] client_score: %d", clientScore);
...
public Action Command_SetTeamLimit(int client, int args) {
	...
	if (g_iTeamCount < g_Cvar_JoinLimit.IntValue) {
		...
		for (int j = 1; j <= playerOverLimit; j++) {
			for (int k = 0; k < teamRedSpec; k++) {
				if (scoreTableRed[k][0] != 0) {
					TF2_ChangeClientTeam(scoreTableRed[k][0], TFTeam_Spectator);
				}
			}
			for (int l = 0; l < teamBlueSpec; l++) {
				if (scoreTableBlue[l][0] != 0) {
					TF2_ChangeClientTeam(scoreTableBlue[l][0], TFTeam_Spectator);
				}
			}
		}
